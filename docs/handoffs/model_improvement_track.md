...
- 2026-06-17 · §6.5 g4 dispersion lever — `research-analyst` brief (`/tmp/researcher_g4_dispersion.md`) + INDEPENDENTLY VERIFIED by gate-path replication. The under-dispersion of the withheld SkewNormal volume cells is NOT a missing mechanism: the existing served scale+skew calibration (`fit_skewnorm_dispersion_skew`) ALREADY clears g4 for WNBA PTS (`pit_ks` 0.077→0.0153) and REB (0.088→0.0328), but is GATED OFF — production fits it on the VAL fold (`pipeline.py:1538`) and reverts to scale-only `(c0, 0)` unless skew beats scale-only by ≥ the global `_DISPERSION_SKEW_MIN_GAIN` = 0.008. The binding axis is SKEW (fit drives `s` to the ±3 clamp), not scale (a global/conditional `c` buys < 0.002 — conditional/heteroscedastic recalibration is NOT worth building). NBA AST is `s`-clamped borderline (0.135→0.0518, just over threshold 0.05) → escalate to a §6.6 centered-parametrization SkewNormal. WNBA AST (0.110→0.070) is an irreducible 38%-zero floor on a `global_mean < 2` cohort → mis-routed onto the SN branch, belongs on the count/ZINB branch (a route, not a dispersion knob). NFL volume additionally fails g1 (book wall). Verification reproduced the gate's decoded `pit_ks` to 3 dp on 4/5 cells (the `none`-strategy NBA FGA replication didn't reproduce — left to the brief, a step-2 cell anyway). Lesson: a deterministic A/B is blind to this lever (it skips the served calibration), so confirm on a real-HPO scorecard. · next: §6.5 implement — replace the global `_DISPERSION_SKEW_MIN_GAIN` with a per-cell honest val→test discount to re-admit the gated skew fit (training-side only, no inference change; cites [Kuleshov 2018 arXiv:1807.00263], [Romano 2019 CQR arXiv:1905.03222] for the borderline cells)
