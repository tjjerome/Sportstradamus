...
- 2026-06-17 · QW-1 cache regen STARTED via concat-backfill (cheaper than full rebuild, operator's call). WNBA (18 cells) + NFL (20 cells) cached matrices backfilled with `OppTotal`/`Spread`/`GameTotal`/`Blowout` reconstructed per (Player, Date) from the gamelog — validated faithful by cached `Total` == reconstruction at rate 1.0 outside the corruption window; `fillna(0)` order + zstd/index=True match production. NBA CANNOT be concat'd: totals corruption propagated into the date-varying `Player`/`Defense totals gain` regression-slope features (corr corrupt↔corrected 0.925, mean|Δ| ≈ 70% of IQR), which only a per-gameday `get_training_matrix` rebuild reproduces. That cold rebuild is BLOCKED by two pre-existing crashes (cached matrices were built warm/incrementally, cold path unexercised): a zero-std z-score `ZeroDivisionError` in `_compute_comps`/`build_comp_profile` (fix `/(x.std() or np.nan)`, 6 sites nba/nfl/nhl) and `comps z` bootstrapping (cold start has no prior comps → `profile_market` omits the col → KeyError). See [[training-matrix-regen-concat]]. · next: NBA rebuild as a deliberate unit — resolve comps bootstrapping + zero-std guard, verify cold-rebuilt comps match warm before trusting the matrix
