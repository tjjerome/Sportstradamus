...
**Fitting half — turn one de-vigged point into a distribution. Mechanism BUILT (book-input track);
the within-player skew prior was A/B-REFUTED and gated off** (`_BOOK_SKEW_PRIOR_ENABLED=False`, so a
meditate bakes `book_skew=0` and production is unchanged) — the shape-borrow stays wired + tested at
`book_skew=0` for a future ladder-fit source. (a) Replace proportional
...
a mean — that is why skew matters). **Built, with a circularity correction:** the
recovery is closed-form, not a root-find — `_book_loc_with_skew` + `fused_loc(line=, book_skew=)`
holds the de-vigged point and shifts only the book *mean*; and the skew is sourced from a
**non-circular** per-cell prior — `_within_player_book_skew` pools each player's standardized
residuals of *realized* outcomes and maps the pooled skewness to a SkewNormal α
(`skewnorm_alpha_from_skewness`, 2·SE noise floor) — **not** the live model row's `(σ̂, α̂)`.
Borrowing the blended row's own shape into the book would flatter the model in g4 (Ranjan-Gneiting,
Pooling half below). **A/B finding:** the within-player *marginal* residual skew (α≈3–4 on NBA
PTS/REB) over-states the book's *conditional* skew — it shifts the blended mean 8–11% and **worsens**
g4/g2 on every tested cell even at w=0.9 — so this prior is gated off; the right source is a per-line
ladder fit (WS1, once rungs accrue), not the marginal residual. (c) Count tail case (anytime-TD): `λ = −log(1−p)` / NegBin
...
- 2026-06-18 · §6.5 Fitting half BUILT (book-input track), dormant pending the A/B (§7.2) — NOT shipped. (a) power de-vig `no_vig_odds(method="power")` (lopsided-only, `|p_over−0.5|>_DEVIG_LOPSIDED_FLAG`=0.3; proportional on the body, byte-identical at even money). (b) shape-borrow book recovery `_book_loc_with_skew`+`fused_loc(line=, book_skew=)` — closed-form (no root-find), holds the de-vigged median, shifts only the book *mean* — sourced from a **non-circular** within-player skew prior `_within_player_book_skew`→`skewnorm_alpha_from_skewness` (pooled standardized-residual skewness of *realized* outcomes, 2·SE noise floor; NOT the model row, or the blend flatters itself in g4). (c) count-tail reg `_sanitize_book_ev` shrinks a runaway book EV to `μ̂+K·SD` (`_BOOK_EV_MODEL_SD_CAP`, keeps the 5×line corruption trigger). WS1: ingest alt-line ladder capture `archive.add_ladder` (per-rung de-vigged, accrues the §1.1 asset — invisible to the gates, usable only once accumulated). `book_skew` rides the cv path (`stat_calibration.json` + pickle `_build_filedict` + serve `model_prob`→`_blend_with_book`); legacy pickles→0.0 so production serving is byte-unchanged until a meditate run bakes a non-zero prior. Coupling RECORDED (Pooling half): the BLP / learned `w` must be co-fit with the book-shape, never sequenced (Ranjan-Gneiting 2010 Thm 1). Also closed a train/serve parity gap: the training blend (`_fuse_skewnormal`) now threads `book_skew`/`line` like serving, so the scorecard gates the served predictive. TDD throughout. · A/B DONE (deterministic §7.2; NBA PTS/REB/MIN; faithful: invert the baseline blend → re-blend both arms through the production `fused_loc`, book_skew=0 reproduces the CSV to 1e-14): the within-player prior is **REFUTED** — it shifts the blended mean +8/+11/−1% and **worsens g4 on all three** (PTS 0.016→0.063, REB 0.027→0.083, MIN 0.031→0.053) plus g2, even at w=0.9. Root cause: the marginal residual skew (α≈3–4) over-states the book's CONDITIONAL skew, which the model already carries. Gated off (`_BOOK_SKEW_PRIOR_ENABLED=False`) ⇒ meditate bakes `book_skew=0`, production byte-unchanged; the shape-borrow + serving/train-blend plumbing stay wired + tested for a ladder-fit source. WS3 (power de-vig) + WS4 (count-tail reg) stand as committed correctness. · next: let WS1 alt-line ladders accrue, then fit the book skew per-line from the ladder (the marginal residual is the wrong quantity), co-fit with the pool (Ranjan-Gneiting)
