...
**Fitting half — turn one de-vigged point into a distribution. BUILT (book-input track), dormant
pending the A/B (§7.2); the design below is now code, default-off until a meditate run bakes a
non-zero `book_skew` into the pickle.** (a) Replace proportional
...
Pooling half below). (c) Count tail case (anytime-TD): `λ = −log(1−p)` / NegBin
