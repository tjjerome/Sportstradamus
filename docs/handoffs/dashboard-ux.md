...
> Status: ACTIVE — Games surface (game-first constellation builder, deduped home-vs-away slate); next P6 deep-dive v2 + swap
...
- **P8 — assets + celestial polish.** Goal: player/team assets, ambient slot scaffolding +
  ...
  P-stage styling (plain expander + grouped chips) and is the first restyle target. Acceptance:
...
- 2026-06-15 · Tonight "Top edge" fix; constellation stays Kelly ✔ (owner-directed, two turns) · The owner first asked to **fix Tonight's "Top edge" caption** (it read `group["Projection"].max()` — the renamed stat-MEAN — and printed it as "Top edge: +25.50", meaningless) **and** to switch the constellation universe + star size from Kelly to "all legs with an edge". I built the second half (`constellation`/`satellite_picker`/`games` filtered + sized on `Model EV` − 1, plus a `dashboard/legs.MODEL_EV_FLOOR = 1.0` constant) — but it's a **no-op set-wise**: `Kelly > 0 ⟺ Model EV > 1` (Boost > 1), so it selects the identical legs and only distorts star *size* by boost. On seeing that, the owner said **keep Kelly** for both the filter and the size. So that work was **fully reverted to HEAD** (`git checkout HEAD --` the 8 files); the only change that landed is the Tonight caption, now showing the best **`Model EV` − 1** (betting edge vs the DFS payout) in the game as a percent. This *supersedes* the §6/§10 P4-era "Tonight top-edge MISLABELS `Model EV` (= stat mean)" note — post-`038bb30` rename the mislabelled column was `Projection`. · commit `30c3b66` · gates ruff ✓ / golden ✓ (1794 pass; same 3 out-of-footprint reds — `test_correlate.py:210` corr-parquet pollution + the real-NBA `get_training_matrix` digest) / integration ✓ (14) · refactoring-specialist not re-run — the sole net change (`tonight.py`, 4 lines) was already in the round-2 specialist scope and is unchanged since; the other 8 files are reverts to HEAD (no net change) · not pushed.
