...
| `constellation_silhouettes` | Games constellation decoration layer (the template's `image` in `data/config/constellation_shapes.json`, drawn by `constellation_slate.add_decoration`) | Clip art filtered to faint light-blue drawings (`data/assets/constellations/`) on 99 of 101 templates; `the-chain-gang` and `the-crossover` draw their outline alone | Art for those two; better fits for `the-kicking-tee` (a golf tee) and `the-wristbands` (a headband) | [`constellation-art`](handoffs/constellation-art.md) lane — openclipart, Wikimedia Commons, game-icons.net | CC0 / public domain, or CC BY 3.0 credited under the Games map; source + artist + licence per image in the lane's `manifest.json` | ≤ 600 px RGBA PNG, tinted `#7FAAE8`, blurred, translucent | done |
