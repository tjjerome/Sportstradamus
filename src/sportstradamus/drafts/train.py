...
def meditate():
    ...
    model = LGBMClassifier(learning_rate=0.1, n_estimators=500, max_depth=-1, objective="binary")
    ...
    preco = precision_score((y_test == 1).astype(int), y_pred[:, 1])
    precu = precision_score((y_test == -1).astype(int), y_pred[:, 0])
    y_pred = y_pred[:, 1] - y_pred[:, 0]
    acc = accuracy_score(y_test[np.abs(y_pred) > 0], y_pred[np.abs(y_pred) > 0])
