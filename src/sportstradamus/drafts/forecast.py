...
for data_str in tqdm(data_list, desc="Loading Data Files", unit="file"):
    if df.empty:
        ...
        df = pd.concat([df, pd.read_csv(pkg_resources.files(data) / data_str, index_col=(0, 1))])
...
for week in np.arange(1, 18):
    ...
    df["NumSpikes"] = df["NumSpikes"] + ((df[f"W{week}_Points"] - mu) / sig > 1.91).astype(int)
# %%
X = df.iloc[:, :36]
...
param_dict = {
    ...
    "min_sum_hessian_in_leaf": ["float", {"low": 1e-2, "high": len(X) * 0.8 / 1000, "log": True}],
...
lgblss.plot(X_test, parameter="rate", plot_type="Feature_Importance")

# %%
# Partial Dependence Plot of rate parameter
lgblss.plot(
    X.loc[(X["NumQB"] < 3) & (X["NumRB"] > 4)],
    parameter="rate",
    feature="PriceRB",
...
ax1.hist(X, bins=n_bins, color="tab:orange", alpha=0.2)
