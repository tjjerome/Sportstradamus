...
for y in rosters:
    depth_df = pd.read_csv(
        f"https://github.com/nflverse/nflverse-data/releases/download/weekly_rosters/roster_weekly_{y}.csv"
    )
    depth_df = depth_df[["full_name", "team", "position", "week"]]
    depth_df = depth_df.loc[depth_df["week"] == 1]
    depth_df = depth_df.loc[depth_df["position"].isin(["QB", "TE", "WR", "RB"])]
...
weekly = weekly.loc[(weekly["position"].isin(["QB", "WR", "RB", "TE"])) & (weekly["week"] < 18)]
weekly["fantasy_points"] = (
    ...
    + weekly["receiving_2pt_conversions"] * 2
    + weekly["rushing_2pt_conversions"] * 2
    + weekly["passing_2pt_conversions"] * 2
    - weekly["interceptions"]
    - weekly["rushing_fumbles_lost"] * 2
    - weekly["receiving_fumbles_lost"] * 2
    - weekly["sack_fumbles_lost"] * 2
)
weekly = (
    weekly[["player_display_name", "season", "week", "fantasy_points"]]
    .groupby(["season", "week"])
    .apply(lambda x: {i[0]: np.round(i[3], 2) for i in x.values})
)
...
data_list = [
    ...
    if ".csv" in f.name and "Regular" in f.name and "teams" not in f.name
]
# %%
# df = pd.DataFrame()
...
def analyze_teams(teams):
    ...
    with tqdm(total=teams.ngroups, desc="Analyzing Teams", unit="team") as pbar:
        for order, team in teams:
            ...
            QB1WR = team.loc[(team["team"] == QBTeams[0]) & wr_filter, "draft_capital_spent"]
            QB1RB = team.loc[(team["team"] == QBTeams[0]) & rb_filter, "draft_capital_spent"]
            QB1TE = team.loc[(team["team"] == QBTeams[0]) & te_filter, "draft_capital_spent"]
            QB2WR = team.loc[(team["team"] == QBTeams[1]) & wr_filter, "draft_capital_spent"]
            QB2RB = team.loc[(team["team"] == QBTeams[1]) & rb_filter, "draft_capital_spent"]
            QB2TE = team.loc[(team["team"] == QBTeams[1]) & te_filter, "draft_capital_spent"]
            ...
            for week in np.arange(1, 18):
                QB1OPP = team.loc[
                    (team["team"] == schedules[(int(team["season"].max()), week)].get(QBTeams[0])),
                    "draft_capital_spent",
                ]
                QB2OPP = team.loc[
                    (team["team"] == schedules[(int(team["season"].max()), week)].get(QBTeams[1])),
                    "draft_capital_spent",
                ]
                QB3OPP = team.loc[
                    (team["team"] == schedules[(int(team["season"].max()), week)].get(QBTeams[2])),
                    "draft_capital_spent",
                ]
                teamStackOPP = team.loc[
                    (team["team"] == schedules[(int(team["season"].max()), week)].get(StackedTeam)),
                    "draft_capital_spent",
                ]
                ...
                team_df[order][f"W{week}_Points"] = (
                    QBPoints.sum()
                    + WRPoints.sum()
                    + RBPoints.sum()
                    + TEPoints.sum()
    ...
    team_df.to_csv(pkg_resources.files(data) / data_str.replace(".csv", "_teams.csv"))
...
for data_str in tqdm(data_list, desc="Loading Data Files", unit="file"):
    ...
    df["team"] = (
        df["player_name"].replace(".", "").map(rosters[int(df.iloc[0]["season"])]).fillna("None")
    )
    df["division"] = df["team"].map(divisions)
    df["bye"] = df["team"].map(byes[int(df.iloc[0]["season"])])

    for week in np.arange(1, 18):
        df[f"player_points_{week}"] = (
            df["player_name"]
            .replace(".", "")
            .map(weekly[(int(df.iloc[0]["season"]), week)])
            .fillna(0)
            .infer_objects(copy=False)
        )

    df["draft_capital_spent"] = np.round(fit[0] * np.log(df["overall_pick_number"]) + fit[1], 2)
    df["projection_draft_capital"] = np.round(
        fit[0] * np.log(df["projection_adp"].replace(0, 216)) + fit[1], 2
    )
    ...
    df["days_until_season_start"] = (
        datetime.strptime("2022-09-08", "%Y-%m-%d") - draft_date
    ).dt.days
    df.loc[df["days_until_season_start"] > 200, "days_until_season_start"] = (
        df.loc[df["days_until_season_start"] > 200, "days_until_season_start"] - 363
    )
