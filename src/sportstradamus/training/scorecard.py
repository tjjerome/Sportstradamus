...
@dataclass(frozen=True)
class CalibrationModeFit:
    """One post-hoc calibration mode fit on a fixed served SkewNormal predictive.

    ``mode`` is the calibration-axis label the combination search reports; ``c`` / ``s`` are the
    fitted scale multiplier and additive skew shift; ``pit_ks`` is the resulting Gate-4
    randomized-PIT KS of the served predictive under ``(c, s)``.
    """
...
def sweep_calibration_modes(
    mean: np.ndarray,
    sigma: np.ndarray,
    skew: np.ndarray,
    y: np.ndarray,
) -> list[CalibrationModeFit]:
    """Fit the four post-hoc calibration modes on one *fixed* trained predictive (no retrain).

    The combination search ranks a cell's calibration corner by Gate-4 headroom across
    ``{none, dispersion, skew-joint, skew-sequential}``; because every mode is a transform of
    the same served arrays (mean held fixed, see :func:`_served_sn_pit_ks`), the whole sweep is
    free — this is what collapses the calibration axis out of the training loop. Each fit carries
    its ``(c, s)`` and Gate-4 KS; the caller picks the minimum-KS mode (subject to the trial's
    Gate-1/5 passing) and forms the ship slack from the trained scorecard row.
    """
    mean = np.asarray(mean, dtype=float)
    sigma = np.asarray(sigma, dtype=float)
    skew = np.asarray(skew, dtype=float)
    ...
    c_disp = fit_skewnorm_dispersion_c(mean, sigma, skew, y)
    c_joint, s_joint = fit_skewnorm_dispersion_skew(mean, sigma, skew, y, joint=True)
    c_seq, s_seq = fit_skewnorm_dispersion_skew(mean, sigma, skew, y, joint=False)
    params = {
        "none": (1.0, 0.0),
        "dispersion": (c_disp, 0.0),
        "skew-joint": (c_joint, s_joint),
        "skew-sequential": (c_seq, s_seq),
    }
    return [
