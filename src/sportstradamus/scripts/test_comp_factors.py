...
players = players.loc[players["position"].isin(["QB", "RB", "WR", "TE"])]
...
for position in ["QB", "RB", "WR", "TE"]:
    ...
    for year in range(2020, 2024):
        ...
        ppg = (
            NFL.gamelog.loc[NFL.gamelog.season == year]
            .groupby("player display name")["fantasy points underdog"]
            .mean()
            .to_dict()
        )
        ...
        if os.path.exists(playerFolder):
            ...
            playerProfile.loc[playerProfile.position == "QB", "less_grades_pass_diff"] = (
                playerProfile.loc[playerProfile.position == "QB", "less_grades_pass"]
            ...
            positionProfile = positionProfile.loc[
                ...
                >= positionProfile[filterStat[position]].max() * 0.1
            ]
            ...
            positionProfile.rename(
                columns={col: f"{year}_{col}" for col in positionProfile.columns}, inplace=True
            )
            position_df = position_df.join(positionProfile, how="outer")

    C = (
        position_df.corr(numeric_only=True)
        .replace(np.inf, 0)
        .replace(-np.inf, 0)
        .replace(np.nan, 0)
        .clip(-1, 1)
