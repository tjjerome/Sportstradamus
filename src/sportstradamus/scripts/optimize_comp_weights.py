    ...
    for market in player_opp_z_lookup:
        ...
        for player, games in game_data.items():
            ...
            for opp, actual_z in games:
                ...
                for comp, dist in zip(comp_names, comp_dists, strict=False):
                    if comp == player:
                        continue
                    key = (comp, opp)
                    if key in opp_z:
...
def prepare_nba(stats_obj, features_by_pos, target_markets):
    ...
    for position in stats_obj.positions:
        ...
        pos_players = [
            p for p, v in playerDict.items() if v["POS"] == position and p in playerProfile.index
        ]
        ...
        lookups = precompute_market_lookups(
            pos_gamelog, "PLAYER_NAME", "OPP", "GAME_DATE", target_markets, "POS", min_games=10
        )
...
def prepare_wnba(stats_obj, features_by_pos, target_markets):
    ...
    for position in stats_obj.positions:
        ...
        lookups = precompute_market_lookups(
            pos_gamelog, "PLAYER_NAME", "OPP", "GAME_DATE", target_markets, "POS", min_games=8
        )
...
def main():
    ...
    parser.add_argument(
        "--save", action="store_true", help="Overwrite playerCompStats.json with optimized weights"
    )
    parser.add_argument(
        "--diagnostic-only",
        action="store_true",
    ...
    if "NBA" in args.leagues:
        ...
        for position in pos_data:
            ...
            diag = run_per_feature_diagnostic(
                z_profile, opp_z, games, positions, position, features
            )
        ...
        for league in args.leagues:
            ...
            for position in optimized[league]:
                ...
                for feature in new:
                    ...
                    print(f"    {feature:40s}  {old_val!s:>5s} -> {new_val:>5.1f}{delta}")
