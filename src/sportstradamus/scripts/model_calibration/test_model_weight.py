...
def load_market(league, market):
    ...
    return {
        'result': result, 'line': line, 'book_ev': book_ev,
        'model_ev': model_ev, 'gate': gate, 'alpha': alpha,
        'r': r, 'nb_p': nb_p, 'cv': cv, 'dist': dist, 'hist_gate': hist_gate,
    }
    ...
    if dist in ("NegBin", "ZINB"):
        ...
        def loss(c):
            ...
            if gate is not None:
                ...
                mu = model_ev
                crps = np.mean(result * (2 * F_y - 1) - mu * (2 * F_y_a1 - 1)
...
def eval_accuracy(w, data, use_dispersion_cal=False):
    ...
    if base_dist == "NegBin":
        r_blend, p_blend, gate_blend = fused_loc(w, data['model_ev'], data['book_ev'],
                                                  data['cv'], "NegBin", r=data['r'], **zi_kwargs)
        weighted_mean = r_blend * (1 - p_blend) / p_blend
        under = get_odds(data['line'], weighted_mean, dist, r=r_blend, gate=gate_blend)
    else:
        alpha_blend, beta_blend, gate_blend = fused_loc(w, data['model_ev'], data['book_ev'],
                                                         data['cv'], "Gamma", alpha=data['alpha'], **zi_kwargs)
...
def objective_nll(w, data, clamp=-20):
    ...
    if base_dist == "NegBin":
        r_blend, p_blend, g_blend = fused_loc(w, data['model_ev'], data['book_ev'],
                                               data['cv'], "NegBin", r=data['r'], **zi_kwargs)
        ...
        alpha_bl, beta_bl, g_blend = fused_loc(w, data['model_ev'], data['book_ev'],
                                                data['cv'], "Gamma", alpha=data['alpha'], **zi_kwargs)
...
def objective_crps(w, data):
    ...
    if base_dist == "NegBin":
        ...
        if has_gate and g_blend is not None:
            ...
            crps_vals = data['result'] * (2 * F_y - 1) - mu_bl * (2 * F_y_a1 - 1) \
...
def optimize_weight(data, objective_fn, name=""):
    ...
    res = minimize(lambda w: objective_fn(w[0], data), [0.5],
                   bounds=[(0.05, 0.9)], tol=1e-8, method='TNC')
    return res.x[0]
...
def eval_accuracy_2w(w_mean, w_shape, data):
    ...
    if base_dist == "NegBin":
        ...
        mu = np.exp(w_mean * np.log(np.clip(data['model_ev'], 1e-9, None)) +
        ...
        gate_blend = None
        if has_gate:
            gate_blend = w_mean * np.asarray(data['gate'], dtype=float) + (1 - w_mean) * data['hist_gate']
        ...
        blended_mean = (w_mean * data['model_ev'] * inv_var_m + (1 - w_mean) * data['book_ev'] * inv_var_b) / total_inv_var
...
def objective_nll_2w(params, data, clamp=-20):
    ...
    if base_dist == "NegBin":
        ...
        blended_alpha = w_shape * model_alpha + (1 - w_shape) * book_alpha
        blended_beta = blended_alpha / blended_mean
        base_logpdf = np.clip(gamma_dist.logpdf(data['result'], blended_alpha, scale=1/blended_beta), clamp, 0)
        if has_gate:
            gate_blend = w_mean * np.asarray(data['gate'], dtype=float) + (1 - w_mean) * data['hist_gate']
            loglik = np.where(
                data['result'] == 0,
...
def main():
    ...
    for league, market in markets:
        ...
        print(f"  mean_model_ev={data['model_ev'].mean():.2f}, mean_book_ev={data['book_ev'].mean():.2f}")
        if data['alpha'] is not None:
            print(f"  mean_model_alpha={data['alpha'].mean():.2f}, book_alpha={1/data['cv']**2:.2f}")
        ...
        try:
            ...
            if data_dc['alpha'] is not None:
                print(f"  corrected_alpha={data_dc['alpha'].mean():.2f} (was {data['alpha'].mean():.2f})")
