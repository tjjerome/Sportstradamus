    ...
    for platform in payout_table:
        ...
        for league in ["NBA", "NFL", "NHL", "MLB", "All"]:
            ...
            if not df.empty:
                ...
                model_threshold = (
                    np.argmax(
                        (mean_profit - np.nanmean(mean_profit)) / np.nanstd(mean_profit) >= 1,
                        axis=1,
                    )
                    * n
                    + 1
                ...
                ax = sns.heatmap(
                    pd.DataFrame(
                        mean_profit,
                        columns=np.round(np.arange(1, 6, n), 1),
                        index=np.round(np.arange(1, 2, n), 1),
