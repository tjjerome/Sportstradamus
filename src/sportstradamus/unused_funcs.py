def find_bets(
    ...
    if len(current_group) == bet_size:
        ...
        if p > 1.5:
            pb = (
                payout
                * (
                    3 * multivariate_normal.cdf(book_odds[current_group], np.zeros(bet_size), SIG)
                    + np.product(norm.cdf(book_odds[current_group]))
                )
                / 4
            )
            units = (p - 1) / (payout - 1) / 0.05
            if pb > 1.01 and units > 0.9:
                units = (p - 1) / (payout - 1) / 0.05
                parlay = info | {
                    "Model EV": p,
                    "Books EV": pb,
                    ...
                    "Markets": [
                        (market, "Under" if leg["Bet"] == "Over" else "Over")
                        if i == 2 and "vs." in leg["Player"]
                        else (market, leg["Bet"])
                        for leg in bet
                        for i, market in enumerate(leg["cMarket"])
                    ],
                    "Fun": np.sum(
                        [
                            3
                            - (np.abs(leg["Line"]) / stat_std.get(league, {}).get(leg["Market"], 1))
                            if ("H2H" in leg["Desc"])
                            else 2
                    ...
                    "Bet Size": bet_size,
                    "Mean EV": np.mean(
                        EV[np.ix_(current_group, current_group)][np.triu_indices(bet_size, 1)]
                    ),
                }
    ...
    if len(current_group) == 0:
        for i in range(C.shape[0]):
            searched.append(i)
            search, stop_search = find_bets(
                C, M, EV, model_odds, book_odds, bet_size, df, info, [i], searched, pbar
            )
            # if pbar:
            #     pbar.update(1)
            if stop_search:
