...
# Plain right-aligned mono style, as a JS object literal for the heatmap expression's
# null / neutral branches (st_aggrid evaluates a cellStyle "function" as `return (expr)`).
...
def _heat_expr(bg: str) -> str:
    return (
        "{'backgroundColor':'" + bg + "','color':'" + _HEAT_TEXT
        + "','textAlign':'right','fontFamily':'IBM Plex Mono, monospace'}"
    )


def _heatmap_cellstyle(values: pd.Series, center: float) -> dict:
    """A diverging cellStyle expression over ``params.value``, bounds baked from the column.
    ...
    expr = (
        "params.value==null||isNaN(params.value) ? " + _RIGHT_EXPR
        + " : params.value <= " + repr(lo2) + " ? " + _heat_expr(_NEG_STRONG)
        + " : params.value <= " + repr(lo1) + " ? " + _heat_expr(_NEG)
        + " : params.value < " + repr(hi1) + " ? " + _RIGHT_EXPR
        + " : params.value < " + repr(hi2) + " ? " + _heat_expr(_POS)
    ...
    *,
    ...
    for col in numeric_cols:
        ...
        if col == heatmap_col:
            ...
            cell_style = {"textAlign": "right", "fontFamily": _MONO}
