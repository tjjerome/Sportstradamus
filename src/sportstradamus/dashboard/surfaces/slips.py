...
with tab_parlays:
    ...
    if _sport != "All" and not parlays.empty and "League" in parlays.columns:
...
with tab_pickem:
    ...
    # Apply global sport switch pre-filter
    _sport = st.session_state.get("sport", "All")
    if _sport != "All" and not entries.empty and "League" in entries.columns:
        entries = entries.loc[entries["League"] == _sport]
