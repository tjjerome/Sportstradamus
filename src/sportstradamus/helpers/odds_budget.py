...
from sportstradamus.helpers.io import ODDS_USAGE_LEDGER_PATH
...
class BudgetDecision(NamedTuple):
    ...
    reason: str  # "ok" | "partial" | "floor_reserve" | "no_fit"
    reserve: float
    per_slot: float
    spendable: float
...
def broad_run_allowance(
    ...
    handed out in ``league_priority`` order while each league's estimated
    run cost still fits.
    """
    ...
    allowance = per_slot
    allowed: list[str] = []
    for league in cfg["league_priority"]:
        ...
        if cost <= allowance:
            ...
            allowance -= cost
    if set(allowed) == set(active_leagues):
        reason = "ok"
    elif allowed:
        reason = "partial"
    else:
