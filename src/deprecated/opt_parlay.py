...
from sportstradamus.helpers import Archive, get_odds, remove_accents, stat_cv, stat_dist
...
stats = {"MLB": mlb, "NBA": nba, "NHL": nhl, "NFL": nfl}
...
for league in ["NBA"]:  # parlays.League.unique():
    ...
    book_results = []
    for _n, row in tqdm(
        league_df.iterrows(), desc=f"Checking {league} parlays", total=len(league_df)
    ):
        legs = row[[col for col in parlays.columns if "Leg " in col and row[col] != ""]]
        if legs.str.contains(" vs. ").any() or legs.str.contains(r" \+ ").any():
            continue
        bet_size = len(legs)
        date = row["Date"]
        teams = row["Game"].split("/")
        players = [
            " ".join(remove_accents(player).split(" ")[:-1])
            for player in row["Players"].split(", ")[:bet_size]
        ]
        for i in range(bet_size):
            test_team = stats[league].gamelog.loc[
                (stats[league].gamelog[log_strings[league]["date"]].str[:10] == date)
                & (stats[league].gamelog[log_strings[league]["player"]] == players[i]),
                log_strings[league]["team"],
            ]
        ...
        book_odds = [
            get_odds(
                lines[i],
                evs[i],
                stat_dist.get(league, {}).get(markets[i], "Gamma"),
                cv=stat_cv[league][markets[i]],
            )
            for i in range(bet_size)
        ]
        ...
        model_results.append({"P": p, "C": corr_p, "R": int(row.Misses == 0)})
        if not any(np.isnan(evs)):
            book_results.append({"P": pb, "C": corr_pb, "R": int(row.Misses == 0)})
    ...
    corr_modifiers[league] = {"Model": res1.x[0], "Books": res2.x[0]}
