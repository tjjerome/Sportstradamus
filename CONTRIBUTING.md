...
| Update the season start date for a league | `stats/{league}.py` → `Stats{League}.season_start` — training/gamelog semantics only; live polling + update gates derive activity from the Odds API events feed (`helpers/odds_budget.py:league_is_live`, snapshot at `data/runtime/league_activity.json`) |
